use quickbridge_core::{SeekSupport, SourceInspection, SourceMetadata};
use reqwest::{
...
pub async fn inspect_source(url: &Url) -> SourceInspection {
    let mut metadata = SourceMetadata::new(filename_from_url(url.as_str()), None);
...
fn finish_inspection(mut metadata: SourceMetadata, seek_support: SeekSupport) -> SourceInspection {
    if metadata.filename().is_empty() {
        metadata = SourceMetadata::new("Unknown", metadata.size_bytes());
    }

    SourceInspection::new(metadata, seek_support)
}
...
fn apply_response_metadata(metadata: &mut SourceMetadata, response: &reqwest::Response) {
    let fallback_name = metadata.filename().to_string();
    ...
    *metadata = SourceMetadata::new(filename, size_bytes.or(metadata.size_bytes()));
}
...
mod tests {
    use super::{
        SeekSupport, classify_seek_support_parts, filename_from_url,
