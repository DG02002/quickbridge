...
async fn run_osascript_capture(lines: &[&str], args: &[&str]) -> Result<String> {
    ...
    let output = command
        .output()
        .await
        .map_err(|source| RuntimeError::QuickTimeScript { source })?;

    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        return Err(RuntimeError::QuickTimeControl {
...
fn parse_playback_status(output: &str) -> Result<PlaybackStatus> {
    ...
    let (playing, current_time) = output
        .split_once('|')
        .ok_or(RuntimeError::QuickTimeUnexpectedStatus)?;
    let current_time = current_time
        .parse::<f64>()
        .ok()
        .and_then(Timecode::from_seconds_f64)
        .ok_or(RuntimeError::QuickTimeInvalidPlayhead)?;

    let playing = match playing {
        "true" => true,
        "false" => false,
        _ => return Err(RuntimeError::QuickTimeInvalidPlaybackFlag),
    };
